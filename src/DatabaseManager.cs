...
public class DatabaseManager
{
    ...
    public void DecryptAndUpdateBiodata(string key)
    {
        try
        {
            OpenConnection();
            ...
            using (MySqlDataReader reader = selectCmd.ExecuteReader())
            {
                ...
                foreach (var record in records)
                {
                    string decryptedNIK = crypto.Decrypt(record.NIK, key);
                    string decryptedNama = crypto.Decrypt(record.Nama, key);
                    string decryptedTempatLahir = crypto.Decrypt(record.TempatLahir, key);
                    string decryptedGolonganDarah = crypto.Decrypt(record.GolonganDarah, key);
                    string decryptedAlamat = crypto.Decrypt(record.Alamat, key);
                    string decryptedAgama = crypto.Decrypt(record.Agama, key);
                    string decryptedPekerjaan = crypto.Decrypt(record.Pekerjaan, key);
                    string decryptedKewarganegaraan = crypto.Decrypt(record.Kewarganegaraan, key);
